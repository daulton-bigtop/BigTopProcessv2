...
const FULL_TEMPLATE = {
  ...
  steps: [
    ...
    },
    {
      ...
      lane: "Finance",
      type: "milestone",
      next: [{ to: "scope_prereq_join", label: "Finance ready" }]
    },
    {
    ...
    },
    {
      ...
      lane: "Project Management",
      type: "task",
      next: ["pre_scope_materials_received_decision"]
    },
    {
      id: "pre_scope_materials_received_decision",
      label: "Required materials received before scope?",
      lane: "Project Management",
      type: "decision",
      next: [
        { to: "preassigned_production_slot", label: "Yes" },
        { to: "pm_parallel_ready", label: "No" }
      ]
    },
    {
      id: "preassigned_production_slot",
      label: "Preassign production slot (pre-scope exception)",
      lane: "Project Management",
      type: "milestone",
    ...
    },
    {
      ...
      lane: "Project Management",
      type: "milestone",
      next: [{ to: "scope_prereq_join", label: "PM ready" }]
    },
    {
    ...
    },
    {
      ...
      label: "Scope prerequisites complete (contract + engineering + finance + PM)",
      lane: "Sales",
      type: "milestone",
      next: ["scope_project"]
    },
    ...
    },
    {
      ...
      label: "Was a production slot preassigned pre-scope?",
      lane: "Project Management",
      type: "decision",
      next: [
        { to: "use_preassigned_production_slot", label: "Yes" },
        { to: "assign_production_slot", label: "No" }
      ]
    },
    {
      id: "use_preassigned_production_slot",
      label: "Use preassigned production slot",
      lane: "Project Management",
      type: "milestone",
...
const BASE_STEP_IDS_DESC = [...STEP_META.keys()].sort((a, b) => b.length - a.length);

const RACI_EDIT_PIN = "0000";
const RACI_STORAGE_KEY = "bigTopRaciOverridesV1";
const RACI_ROLES = [
  { key: "sales", label: "Sales" },
  { key: "pm", label: "Project Manager" },
  { key: "engineering", label: "Engineering / CAD" },
  { key: "production", label: "Production" },
  { key: "shipping", label: "Shipping / Receiving" },
  { key: "finance", label: "Finance" },
  { key: "installer", label: "Installer" },
  { key: "customer", label: "Customer" }
];

const RACI_PHASE_LIBRARY = {
  pre_lead: {
    title: "Pre-PO | Lead Generation & Qualification",
    description: "Sales remains the primary point of contact pre-PO and leads opportunity qualification and pipeline development.",
    assignments: {
      sales: { code: "R/A", note: "Identifies and qualifies opportunities; owns pipeline quality." },
      pm: { code: "I", note: "Informed to anticipate project load and complexity." },
      engineering: { code: "I", note: "Aware of potential design support needs." },
      production: { code: "I", note: "Aware of likely production demand." },
      shipping: { code: "I", note: "Aware of potential logistics impact." },
      finance: { code: "I", note: "Aware for forecasting and payment planning." },
      installer: { code: "I", note: "Aware if install delivery model may apply." },
      customer: { code: "C", note: "Provides initial project intent, constraints, and goals." }
    }
  },
  pre_needs: {
    title: "Pre-PO | Needs Assessment & Solution Design",
    description: "Sales leads customer discovery and validates solution fit with PM/Engineering input before quoting.",
    assignments: {
      sales: { code: "R/A", note: "Leads discovery, confirms requirements, and aligns scope to budget." },
      pm: { code: "C", note: "Confirms feasibility and execution alignment." },
      engineering: { code: "R", note: "Provides layouts/technical validation and feasibility support." },
      production: { code: "I", note: "Informed of implications for build approach." },
      shipping: { code: "I", note: "Aware of preliminary freight and handling needs." },
      finance: { code: "I", note: "Reviews payment term implications." },
      installer: { code: "C", note: "Provides install feasibility input when needed." },
      customer: { code: "C", note: "Provides requirements and site constraints for validation." }
    }
  },
  pre_quote: {
    title: "Pre-PO | Quotation & Proposal Development",
    description: "Sales owns consolidated quoting while PM/Shipping/Engineering contribute install, logistics, and technical inputs.",
    assignments: {
      sales: { code: "R/A", note: "Owns shelter pricing and proposal package." },
      pm: { code: "C/R", note: "Coordinates accessory/install pricing and bid support." },
      engineering: { code: "C", note: "Supplies drawings/spec inputs for quote accuracy." },
      production: { code: "I", note: "Informed for lead-time or cost implications." },
      shipping: { code: "C/R", note: "Provides freight estimates and assumptions." },
      finance: { code: "C", note: "Reviews pricing structure and payment terms." },
      installer: { code: "R", note: "Returns install bids and field execution assumptions." },
      customer: { code: "C", note: "Reviews proposal and requests fit-to-budget revisions." }
    }
  },
  pre_contract: {
    title: "Pre-PO | Contract / Order Confirmation",
    description: "Sales secures PO/contract and confirms commercial terms before execution transition.",
    assignments: {
      sales: { code: "R/A", note: "Secures PO/contract and validates scope and terms." },
      pm: { code: "C", note: "Reviews risk, schedule, and execution readiness." },
      engineering: { code: "I", note: "Confirms any drawing/design commitments." },
      production: { code: "I", note: "Prepares for production schedule alignment." },
      shipping: { code: "I", note: "Validates delivery commitments and feasibility." },
      finance: { code: "C", note: "Validates payment schedule and credit terms." },
      installer: { code: "I", note: "Informed for install feasibility and calendar planning." },
      customer: { code: "R", note: "Issues PO/contract and confirms commercial acceptance." }
    }
  },
  post_kickoff: {
    title: "Post-PO | Project Kickoff & Planning",
    description: "PM becomes primary execution contact after order acceptance and leads cross-functional kickoff, schedule, and milestones.",
    assignments: {
      sales: { code: "C", note: "Participates for expectation alignment and continuity." },
      pm: { code: "R/A", note: "Leads kickoff, timeline, milestones, and execution orchestration." },
      engineering: { code: "C", note: "Provides design dependencies and timeline input." },
      production: { code: "C", note: "Confirms capacity and readiness." },
      shipping: { code: "C", note: "Provides shipping and coordination timing." },
      finance: { code: "I", note: "Informed for milestone-based billing setup." },
      installer: { code: "C", note: "Informed/consulted on install windows and constraints." },
      customer: { code: "I", note: "Informed of execution plan and key milestones." }
    }
  },
  post_engineering: {
    title: "Post-PO | Engineering & Drawings",
    description: "Engineering/CAD leads deliverables and revisions; PM coordinates approval timing and dependencies.",
    assignments: {
      sales: { code: "I", note: "Informed for customer updates and expectation management." },
      pm: { code: "C", note: "Coordinates review cycles and approval timing." },
      engineering: { code: "R/A", note: "Creates, revises, and finalizes approved drawings." },
      production: { code: "I", note: "Reviews for manufacturability implications." },
      shipping: { code: "I", note: "Prepares logistics assumptions from final design." },
      finance: { code: "I", note: "Monitors timing impact on invoicing milestones." },
      installer: { code: "C", note: "Provides installability feedback where needed." },
      customer: { code: "C", note: "Reviews/approves engineering package and revisions." }
    }
  },
  post_production: {
    title: "Post-PO | Procurement & Production",
    description: "PM oversees integrated schedule while Production executes fabrication and readiness.",
    assignments: {
      sales: { code: "I", note: "Informed of progress for customer communication." },
      pm: { code: "R/A", note: "Oversees schedule, material release, and cross-team coordination." },
      engineering: { code: "C", note: "Confirms design changes impacting production." },
      production: { code: "R/A", note: "Executes fabrication, QA/QC, and readiness checks." },
      shipping: { code: "C", note: "Coordinates packaging and shipment readiness." },
      finance: { code: "I", note: "Validates deposit/progress payments against milestones." },
      installer: { code: "I", note: "Informed to align install readiness and mobilization." },
      customer: { code: "I", note: "Informed of status and target ship/installation windows." }
    }
  },
  post_shipping: {
    title: "Post-PO | Shipping & Logistics",
    description: "Shipping and PM coordinate release, offload readiness, and delivery confirmation.",
    assignments: {
      sales: { code: "I", note: "Communicates delivery expectations to customer." },
      pm: { code: "R/A", note: "Confirms execution readiness and shipment scheduling alignment." },
      engineering: { code: "I", note: "Provides shipping technical data if required." },
      production: { code: "C", note: "Prepares finished goods for safe transport." },
      shipping: { code: "R/A", note: "Manages freight, documents, and delivery confirmation." },
      finance: { code: "I", note: "Triggers/controls invoicing rules around shipment release." },
      installer: { code: "C", note: "Coordinates offload/arrival alignment for install jobs." },
      customer: { code: "C", note: "Confirms receiving availability and handoff readiness." }
    }
  },
  post_install: {
    title: "Post-PO | Installation & Site Execution",
    description: "PM leads installation execution coordination; installer/customer complete site work based on install model.",
    assignments: {
      sales: { code: "I", note: "Maintains relationship updates without direct coordination ownership." },
      pm: { code: "R/A", note: "Coordinates installers, site work, and safety compliance." },
      engineering: { code: "I", note: "Supports as-built adjustments if needed." },
      production: { code: "C", note: "Supports installation readiness through quality confirmation." },
      shipping: { code: "C", note: "Confirms delivery completeness and status." },
      finance: { code: "I", note: "Informed for milestone billing triggers." },
      installer: { code: "R", note: "Executes site installation scope when installer model applies." },
      customer: { code: "C", note: "Participates in site access, acceptance, and install coordination." }
    }
  },
  post_change: {
    title: "Post-PO | Change Orders & Variations",
    description: "Sales captures customer-requested changes while PM leads approval workflow and impact management.",
    assignments: {
      sales: { code: "R", note: "Captures customer requests and communicates change scope." },
      pm: { code: "R/A", note: "Evaluates, documents, and routes change-order approval." },
      engineering: { code: "C", note: "Defines technical design impacts." },
      production: { code: "C", note: "Estimates labor/material schedule impacts." },
      shipping: { code: "I", note: "Confirms logistics implications." },
      finance: { code: "C", note: "Issues/updates billing after change approval." },
      installer: { code: "C", note: "Advises install-site impact and sequencing." },
      customer: { code: "C", note: "Reviews and approves requested variation." }
    }
  },
  post_closeout: {
    title: "Post-PO | Closeout & Handover",
    description: "PM leads project closeout and handover while Finance finalizes invoicing/collections and Sales supports satisfaction.",
    assignments: {
      sales: { code: "C", note: "Supports satisfaction review and relationship continuity." },
      pm: { code: "R/A", note: "Leads closeout, punch list resolution, and handover delivery." },
      engineering: { code: "C", note: "Provides as-built drawings and technical closeout data." },
      production: { code: "C", note: "Supplies final QA/QC and sign-off data." },
      shipping: { code: "C", note: "Confirms all delivery obligations complete." },
      finance: { code: "R/A", note: "Completes final invoicing and collections." },
      installer: { code: "C", note: "Supports final completion documentation as needed." },
      customer: { code: "C", note: "Participates in final acceptance and handover." }
    }
  },
  fallback: {
    title: "Workflow Activity | General",
    description: "Default ownership model inferred from lane ownership and cross-functional review needs.",
    assignments: {
      sales: { code: "C", note: "Supports commercial alignment and customer communication." },
      pm: { code: "C", note: "Supports schedule, coordination, and readiness." },
      engineering: { code: "C", note: "Supports technical fit and drawing validation." },
      production: { code: "I", note: "Informed for build impact." },
      shipping: { code: "I", note: "Informed for logistics impact." },
      finance: { code: "I", note: "Informed for billing/terms implications." },
      installer: { code: "I", note: "Informed for field execution implications." },
      customer: { code: "I", note: "Informed of outcome and next steps." }
    }
  }
};

const STEP_PHASE_KEY = (() => {
  const map = {};
  const assign = (phase, ids) => {
    ids.forEach((id) => {
      map[id] = phase;
    });
  };

  assign("pre_lead", ["lead_received", "qualify_lead", "can_assist"]);
  assign("pre_needs", [
    "need_more_info",
    "request_customer_info",
    "customer_provides_info",
    "feasibility_check_needed",
    "send_feasibility_request",
    "engineering_feasibility_review",
    "feasible_for_site"
  ]);
  assign("pre_quote", [
    "install_required",
    "install_quote_request",
    "pm_create_bidtracer_request",
    "bidtracer_invites_installers",
    "installers_submit_bid",
    "pm_markup_install_bid",
    "sales_receive_install_price",
    "build_structure_quote",
    "customer_review_quote",
    "budget_revision_needed",
    "sales_revise_quote"
  ]);
  assign("pre_contract", ["po_received_decision", "po_received", "closed_lost"]);
  assign("post_kickoff", [
    "post_award_parallel_launch",
    "payment_terms_decision",
    "invoice_50_percent",
    "deposit_received",
    "finance_terms_recorded",
    "finance_parallel_ready",
    "pm_award_notification",
    "pm_schedule_installers_parallel",
    "pre_scope_materials_received_decision",
    "preassigned_production_slot",
    "pm_parallel_ready",
    "contract_negotiation",
    "contract_approved_decision",
    "contract_revision",
    "contract_ready",
    "scope_prereq_join",
    "scope_project",
    "issue_tech_data_sheet"
  ]);
  assign("post_engineering", [
    "engineering_required_decision",
    "engineering_package",
    "customer_engineering_approval",
    "engineering_revision",
    "engineering_ready",
    "cad_sketch",
    "customer_sketch_approval"
  ]);
  assign("post_change", ["change_order"]);
  assign("post_production", [
    "pm_finalize_installer_schedule",
    "production_slot_decision",
    "use_preassigned_production_slot",
    "assign_production_slot",
    "enter_production",
    "order_trucks",
    "finished_goods_ready"
  ]);
  assign("post_shipping", [
    "release_to_ship_gate",
    "hold_for_final_payment",
    "offload_coordination",
    "pm_verify_offload",
    "shipping_verify_customer_receive",
    "ship_order"
  ]);
  assign("post_install", ["installation_owner_decision", "installer_complete_install", "customer_complete_install"]);
  assign("post_closeout", ["final_invoice_required", "issue_final_invoice", "cash_received", "project_closeout"]);

  return map;
})();

const RACI_STEP_OVERRIDES = {
  contract_negotiation: {
    description: "Sales leads final contract and scope terms while PM, Finance, and customer align execution and commercial commitments before scope.",
    assignments: {
      sales: { code: "R/A", note: "Leads commercial negotiation and confirms scope language." },
      pm: { code: "C", note: "Confirms execution dates and operational commitments." },
      finance: { code: "C", note: "Confirms payment terms and invoicing conditions." },
      customer: { code: "C", note: "Reviews and negotiates terms before execution." }
    }
  },
  engineering_revision: {
    description: "Engineering revisions may loop with customer technical stakeholders until design approval is final.",
    assignments: {
      engineering: { code: "R/A", note: "Executes revision and technical coordination." },
      pm: { code: "C", note: "Coordinates timing and impact to downstream schedule." },
      customer: { code: "C", note: "Provides review comments and approval criteria." }
    }
  },
  scope_prereq_join: {
    description: "Scope starts only after contract, engineering, finance, and PM readiness are all complete.",
    assignments: {
      sales: { code: "A", note: "Owns scope release decision after prerequisites complete." },
      pm: { code: "C", note: "Confirms operational readiness and install schedule." },
      engineering: { code: "C", note: "Confirms approved technical package status." },
      finance: { code: "C", note: "Confirms payment/term prerequisite completion." }
    }
  },
  scope_project: {
    description: "Sales owns scope release once all prerequisites are satisfied and transitions execution detail to operations.",
    assignments: {
      sales: { code: "R/A", note: "Releases scoped project package and confirms commercial baseline." },
      pm: { code: "C", note: "Uses scoped package to execute scheduling and delivery plan." }
    }
  },
  change_order: {
    description: "Change orders can occur from PO receipt through pre-install and must be scoped and approved before execution changes.",
    assignments: {
      sales: { code: "R", note: "Captures customer-requested changes and scope intent." },
      pm: { code: "R/A", note: "Coordinates impact review and approval workflow." },
      finance: { code: "C", note: "Updates billing terms tied to approved changes." }
    }
  },
  pre_scope_materials_received_decision: {
    description: "Mid-flow pre-scope decision to determine whether required materials are in-hand early enough to justify a preassigned production slot.",
    assignments: {
      pm: { code: "R/A", note: "Evaluates readiness and records decision." },
      production: { code: "C", note: "Confirms material availability and slot impact." },
      sales: { code: "I", note: "Informed when pre-scope slot strategy changes delivery approach." }
    }
  },
  preassigned_production_slot: {
    description: "Preassigned slot is established pre-scope when required materials are already available and PM/Production approve early reservation.",
    assignments: {
      pm: { code: "R/A", note: "Confirms pre-scope reservation and downstream schedule alignment." },
      production: { code: "C", note: "Confirms slot availability and production impact." }
    }
  },
  production_slot_decision: {
    description: "Post-scope checkpoint to determine whether a preassigned slot exists or if standard slot assignment is required.",
    assignments: {
      pm: { code: "R/A", note: "Determines slot path for production entry." },
      production: { code: "C", note: "Validates feasible entry window." }
    }
  },
  hold_for_final_payment: {
    description: "Shipment is held until payment conditions are met unless net terms allow release.",
    assignments: {
      finance: { code: "R/A", note: "Controls payment release gate." },
      shipping: { code: "R", note: "Holds shipment until release confirmation." },
      pm: { code: "C", note: "Coordinates downstream schedule impact." }
    }
  }
};
...
const els = {
  ...
  compassNav: document.getElementById("compassNav"),
  compassDragHandle: document.getElementById("compassDragHandle"),
  compassUp: document.getElementById("compassUp"),
  compassDown: document.getElementById("compassDown"),
  compassLeft: document.getElementById("compassLeft"),
  compassRight: document.getElementById("compassRight"),
  compassCenter: document.getElementById("compassCenter"),
  compassZoomOut: document.getElementById("compassZoomOut"),
  compassZoomIn: document.getElementById("compassZoomIn"),
  compassZoomValue: document.getElementById("compassZoomValue"),
  raciModal: document.getElementById("raciModal"),
  raciCloseBtn: document.getElementById("raciCloseBtn"),
  raciMeta: document.getElementById("raciMeta"),
  raciLockState: document.getElementById("raciLockState"),
  raciPinInput: document.getElementById("raciPinInput"),
  raciUnlockBtn: document.getElementById("raciUnlockBtn"),
  raciSaveBtn: document.getElementById("raciSaveBtn"),
  raciResetBtn: document.getElementById("raciResetBtn"),
  raciPhaseInput: document.getElementById("raciPhaseInput"),
  raciDescriptionInput: document.getElementById("raciDescriptionInput"),
  raciSummaryInput: document.getElementById("raciSummaryInput"),
...
let currentZoom = 1;
let raciOverrides = loadRaciOverrides();
let raciActiveContext = null;
let raciEditUnlocked = false;
const ZOOM_MIN = 0.6;
const ZOOM_MAX = 2;
const ZOOM_STEP = 0.1;
const LANE_HEADER_GROUP_ID = "laneHeaderStickyGroup";
...
function bindEvents() {
  ...
  bindCompassControls();
  bindStickyLaneHeaders();
}

function bindStickyLaneHeaders() {
  if (!els.canvasScroll) {
    return;
  }

  els.canvasScroll.addEventListener("scroll", updateStickyLaneHeaders, { passive: true });
  window.addEventListener("resize", updateStickyLaneHeaders);
}

function bindRaciModalEvents() {
  if (!els.raciModal) {
    return;
  }

  els.raciModal.hidden = true;

  if (els.raciCloseBtn) {
    els.raciCloseBtn.addEventListener("click", closeRaciModal);
  }

  if (els.raciUnlockBtn) {
    els.raciUnlockBtn.addEventListener("click", () => {
      if (!raciActiveContext) {
        return;
      }

      const entered = String(els.raciPinInput ? els.raciPinInput.value : "").trim();
      if (entered !== RACI_EDIT_PIN) {
        showStatus("Invalid PIN. RACI edit mode remains locked.", "error");
        return;
      }

      raciEditUnlocked = true;
      updateRaciEditState();
      showStatus("RACI edit mode unlocked.", "success");
    });
  }

  if (els.raciSaveBtn) {
    els.raciSaveBtn.addEventListener("click", () => {
      if (!raciActiveContext || !raciEditUnlocked) {
        return;
      }

      const baseId = raciActiveContext.baseId;
      raciOverrides[baseId] = {
        phaseTitle: String(els.raciPhaseInput ? els.raciPhaseInput.value : "").trim(),
        description: String(els.raciDescriptionInput ? els.raciDescriptionInput.value : "").trim(),
        summary: String(els.raciSummaryInput ? els.raciSummaryInput.value : "").trim(),
        updatedAt: new Date().toISOString()
      };
      saveRaciOverrides(raciOverrides);
      showStatus(`Saved RACI details for '${baseId}'.`, "success");
    });
  }

  if (els.raciResetBtn) {
    els.raciResetBtn.addEventListener("click", () => {
      if (!raciActiveContext || !raciEditUnlocked) {
        return;
      }

      const baseId = raciActiveContext.baseId;
      delete raciOverrides[baseId];
      saveRaciOverrides(raciOverrides);

      const defaults = buildDefaultRaciProfile(raciActiveContext.step, baseId);
      setRaciFormValues(defaults);
      showStatus(`Reset RACI details for '${baseId}' to default template.`, "success");
    });
  }

  els.raciModal.addEventListener("click", (event) => {
    if (event.target === els.raciModal) {
      closeRaciModal();
    }
  });

  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && els.raciModal && !els.raciModal.hidden) {
      closeRaciModal();
    }
  });
...
function updateRaciEditState() {
  const readOnly = !raciEditUnlocked;

  [els.raciPhaseInput, els.raciDescriptionInput, els.raciSummaryInput].forEach((element) => {
    if (element) {
      element.readOnly = readOnly;
    }
  });

  if (els.raciLockState) {
    els.raciLockState.textContent = readOnly ? "Read-only" : "Editing unlocked";
  }

  if (els.raciUnlockBtn) {
    els.raciUnlockBtn.disabled = !raciActiveContext || !readOnly;
  }

  if (els.raciSaveBtn) {
    els.raciSaveBtn.disabled = !raciActiveContext || readOnly;
  }

  if (els.raciResetBtn) {
    els.raciResetBtn.disabled = !raciActiveContext || readOnly;
  }
}

function openRaciModal(step) {
  if (!step || !els.raciModal) {
    return;
  }

  const baseId = resolveBaseStepId(step.id);
  raciActiveContext = {
    step,
    baseId
  };
  raciEditUnlocked = false;

  if (els.raciPinInput) {
    els.raciPinInput.value = "";
  }

  const profile = getRaciProfileForStep(step);
  setRaciFormValues(profile);

  if (els.raciMeta) {
    els.raciMeta.textContent = `Step: ${step.label} | ID: ${step.id} | Base: ${baseId} | Lane: ${step.lane} | Saves are stored in this browser.`;
  }

  updateRaciEditState();
  els.raciModal.hidden = false;
}
...
function closeRaciModal() {
  ...
  els.raciModal.hidden = true;
  raciActiveContext = null;
  raciEditUnlocked = false;
  updateRaciEditState();
}

function setRaciFormValues(profile) {
  if (els.raciPhaseInput) {
    els.raciPhaseInput.value = profile.phaseTitle || "";
  }
  if (els.raciDescriptionInput) {
    els.raciDescriptionInput.value = profile.description || "";
  }
  if (els.raciSummaryInput) {
    els.raciSummaryInput.value = profile.summary || "";
  }
}

function getRaciProfileForStep(step) {
  const baseId = resolveBaseStepId(step.id);
  const defaults = buildDefaultRaciProfile(step, baseId);
  const custom = raciOverrides[baseId];
  ...
  return {
    phaseTitle: custom.phaseTitle || defaults.phaseTitle,
    description: custom.description || defaults.description,
    summary: custom.summary || defaults.summary
  };
}

function buildDefaultRaciProfile(step, baseId) {
  const phaseKey = STEP_PHASE_KEY[baseId] || inferPhaseKeyForStep(step);
  const phase = RACI_PHASE_LIBRARY[phaseKey] || RACI_PHASE_LIBRARY.fallback;
  const assignments = clone(phase.assignments || {});
  const override = RACI_STEP_OVERRIDES[baseId];

  if (override && override.assignments) {
    mergeAssignments(assignments, override.assignments);
  }

  const laneRole = laneToRoleKey(step.lane);
  if (laneRole) {
    forceResponsible(assignments, laneRole, step.type === "milestone");
  }

  if (step.lane === "Customer") {
    assignments.customer = {
      code: step.type === "decision" ? "R/A" : "R",
      note: "Provides approvals, information, and acceptance actions."
    };
  }

  if (step.lane === "Installer") {
    assignments.installer = {
      code: "R",
      note: "Executes installer-side field scope and confirmations."
    };
    forceResponsible(assignments, "pm", false);
  }

  return {
    phaseTitle: phase.title,
    description: override && override.description ? override.description : phase.description,
    summary: formatRaciSummary(assignments)
  };
}

function mergeAssignments(target, source) {
  Object.keys(source || {}).forEach((roleKey) => {
    target[roleKey] = {
      ...(target[roleKey] || {}),
      ...(source[roleKey] || {})
    };
...
function formatRaciSummary(assignments) {
  return RACI_ROLES.map((role) => {
    const entry = assignments[role.key] || { code: "I", note: defaultNoteForRole(role.key) };
    const code = entry.code || "I";
    const note = entry.note ? ` - ${entry.note}` : "";
    return `${role.label}: ${code}${note}`;
  }).join("\n");
}

function defaultNoteForRole(roleKey) {
  if (roleKey === "sales") {
    return "Commercial and customer alignment support.";
  }
  if (roleKey === "pm") {
    return "Execution and schedule coordination support.";
  }
  if (roleKey === "engineering") {
    return "Technical support and design validation.";
  }
  if (roleKey === "production") {
    return "Build and readiness awareness.";
  }
  if (roleKey === "shipping") {
    return "Logistics and freight awareness.";
  }
  if (roleKey === "finance") {
    return "Payment terms and invoice awareness.";
  }
  if (roleKey === "installer") {
    return "Field execution awareness.";
  }
  return "Stakeholder awareness.";
}

function forceResponsible(assignments, roleKey, accountable) {
  const existing = assignments[roleKey] || {
    code: "I",
    note: defaultNoteForRole(roleKey)
  };

  if (!String(existing.code || "").match(/R|A/)) {
    existing.code = accountable ? "R/A" : "R";
  } else if (accountable && !String(existing.code || "").includes("A")) {
    existing.code = "R/A";
  }

  assignments[roleKey] = existing;
}

function laneToRoleKey(lane) {
  if (lane === "Sales") {
    return "sales";
  }
  if (lane === "Project Management") {
    return "pm";
  }
  if (lane === "CAD/Engineering") {
    return "engineering";
  }
  if (lane === "Production") {
    return "production";
...
function inferPhaseKeyForStep(step) {
  if (step.lane === "Finance") {
    return "post_kickoff";
  }
  if (step.lane === "Production") {
    return "post_production";
  }
  if (step.lane === "Installer") {
    return "post_install";
  }
  if (step.lane === "CAD/Engineering") {
    return "post_engineering";
  }
  if (step.lane === "Project Management") {
    return "post_kickoff";
  }
  if (step.lane === "Sales" && step.type === "end") {
    return "post_closeout";
  }
  return "fallback";
}

function resolveBaseStepId(stepId) {
  if (STEP_META.has(stepId)) {
    return stepId;
  }

  for (const baseId of BASE_STEP_IDS_DESC) {
    if (stepId === baseId || stepId.startsWith(`${baseId}_`)) {
      return baseId;
    }
  }
...
function loadRaciOverrides() {
  try {
    const raw = window.localStorage.getItem(RACI_STORAGE_KEY);
    if (!raw) {
      return {};
    }
...
function saveRaciOverrides(overrides) {
  try {
    window.localStorage.setItem(RACI_STORAGE_KEY, JSON.stringify(overrides || {}));
  } catch (error) {
...
function bindCompassControls() {
  ...
  const quickStep = () => ({
    x: Math.max(150, Math.round(scroll.clientWidth * 0.24)),
    y: Math.max(150, Math.round(scroll.clientHeight * 0.24))
  });

  const holdStep = () => ({
    x: Math.max(70, Math.round(scroll.clientWidth * 0.09)),
    y: Math.max(70, Math.round(scroll.clientHeight * 0.09))
  });

  const moveBy = (xFactor, yFactor, smooth = true) => {
    const step = smooth ? quickStep() : holdStep();
    scroll.scrollBy({
      left: xFactor * step.x,
      top: yFactor * step.y,
  ...
  const attachDirectional = (button, xFactor, yFactor) => {
    if (!button) {
      return;
    }
    ...
    button.addEventListener("pointerdown", (event) => {
      ...
      moveBy(xFactor, yFactor, true);

      holdTimer = window.setTimeout(() => {
        holdInterval = window.setInterval(() => {
          moveBy(xFactor, yFactor, false);
        }, 90);
      }, 280);
    });

    ["pointerup", "pointerleave", "pointercancel", "blur"].forEach((name) => {
      button.addEventListener(name, clearHold);
    });
  };

  attachDirectional(els.compassUp, 0, -1);
  attachDirectional(els.compassDown, 0, 1);
  attachDirectional(els.compassLeft, -1, 0);
  attachDirectional(els.compassRight, 1, 0);

  if (els.compassZoomIn) {
    els.compassZoomIn.addEventListener("click", () => {
      zoomTo(currentZoom + ZOOM_STEP);
    });
  }

  if (els.compassZoomOut) {
    els.compassZoomOut.addEventListener("click", () => {
      zoomTo(currentZoom - ZOOM_STEP);
    });
  }
  ...
  if (els.compassCenter) {
    els.compassCenter.addEventListener("click", () => {
      scroll.scrollTo({
        left: Math.max(0, (scroll.scrollWidth - scroll.clientWidth) / 2),
        top: Math.max(0, (scroll.scrollHeight - scroll.clientHeight) / 2),
        behavior: "smooth"
      });
    });
  }
...
function zoomTo(targetZoom) {
  const bounded = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, targetZoom));
  const rounded = Math.round(bounded * 10) / 10;
  if (Math.abs(rounded - currentZoom) < 0.001) {
    updateZoomUi();
    return;
  }
  ...
  currentZoom = rounded;
  applyChartZoom({
    x: Math.max(0, els.canvasScroll.clientWidth / 2),
    y: Math.max(0, els.canvasScroll.clientHeight / 2)
  });
  updateZoomUi();
}

function applyChartZoom(anchor) {
  ...
  const oldWidth = Number(chart.getAttribute("width")) || baseWidth;
  const oldHeight = Number(chart.getAttribute("height")) || baseHeight;
  const focusX = anchor && Number.isFinite(anchor.x) ? anchor.x : scroll.clientWidth / 2;
  const focusY = anchor && Number.isFinite(anchor.y) ? anchor.y : scroll.clientHeight / 2;

  const ratioX = oldWidth > 0 ? (scroll.scrollLeft + focusX) / oldWidth : 0;
  const ratioY = oldHeight > 0 ? (scroll.scrollTop + focusY) / oldHeight : 0;

  const nextWidth = Math.max(1, Math.round(baseWidth * currentZoom));
  const nextHeight = Math.max(1, Math.round(baseHeight * currentZoom));

  chart.setAttribute("width", String(nextWidth));
  chart.setAttribute("height", String(nextHeight));

  const nextLeft = ratioX * nextWidth - focusX;
  const nextTop = ratioY * nextHeight - focusY;
  const maxLeft = Math.max(0, nextWidth - scroll.clientWidth);
  const maxTop = Math.max(0, nextHeight - scroll.clientHeight);

  scroll.scrollLeft = Math.max(0, Math.min(maxLeft, nextLeft));
  scroll.scrollTop = Math.max(0, Math.min(maxTop, nextTop));
  updateStickyLaneHeaders();
}

function updateZoomUi() {
  if (els.compassZoomValue) {
    els.compassZoomValue.textContent = `${Math.round(currentZoom * 100)}%`;
  }
  if (els.compassZoomIn) {
    els.compassZoomIn.disabled = currentZoom >= ZOOM_MAX;
  }
  if (els.compassZoomOut) {
    els.compassZoomOut.disabled = currentZoom <= ZOOM_MIN;
  }
}

function updateStickyLaneHeaders() {
  const svg = els.chart;
  const scroll = els.canvasScroll;
  if (!svg || !scroll) {
    return;
  ...
  const laneHeaderGroup = svg.querySelector(`#${LANE_HEADER_GROUP_ID}`);
  if (!laneHeaderGroup) {
    return;
  }

  const zoom = currentZoom > 0 ? currentZoom : 1;
  const translateX = scroll.scrollLeft / zoom;
  laneHeaderGroup.setAttribute("transform", `translate(${Math.round(translateX)},0)`);
}

function bindCompassDrag() {
  const nav = els.compassNav;
  const handle = els.compassDragHandle;
  const wrap = document.querySelector(".canvas-wrap");
  const storageKey = "bigTopCompassNavPositionV1";
  ...
  const clampPosition = (left, top) => {
    ...
    const maxLeft = Math.max(8, wrap.clientWidth - navRect.width - 8);
    const maxTop = Math.max(8, wrap.clientHeight - navRect.height - 8);
  ...
  const applyPosition = (left, top) => {
    ...
    nav.style.left = `${Math.round(clamped.left)}px`;
    nav.style.top = `${Math.round(clamped.top)}px`;
  ...
  const savePosition = () => {
    try {
      window.localStorage.setItem(storageKey, JSON.stringify({
        left: parseFloat(nav.style.left),
        top: parseFloat(nav.style.top)
      }));
    } catch (error) {
  ...
  const onPointerMove = (event) => {
    ...
    const nextLeft = event.clientX - wrapRect.left - drag.offsetX;
    const nextTop = event.clientY - wrapRect.top - drag.offsetY;
  ...
  handle.addEventListener("pointerdown", (event) => {
    ...
    const navRect = nav.getBoundingClientRect();
    drag = {
      offsetX: event.clientX - navRect.left,
      offsetY: event.clientY - navRect.top
    };
...
function buildScenario(input) {
  ...
  const preScopeMaterialsDecision = add("pre_scope_materials_received_decision");
  connect(pmPlan, preScopeMaterialsDecision);
  const preassignedPreScope = add("preassigned_production_slot");
  connect(preScopeMaterialsDecision, preassignedPreScope, "Yes");
  const pmReady = add("pm_parallel_ready");
  connect(preScopeMaterialsDecision, pmReady, "No");
  connect(preassignedPreScope, pmReady, "Yes");
  ...
  const prereqJoin = add("scope_prereq_join", {
    label: input.paymentTerms === "net"
      ? "Scope prerequisites complete (contract + engineering + finance terms + PM)"
      : "Scope prerequisites complete (contract + engineering + down payment + PM)"
  });
  connect(contractReady, prereqJoin, "Contract");
  connect(engineeringReady, prereqJoin, "Engineering");
  connect(financeReady, prereqJoin, input.paymentTerms === "net" ? "Net Terms" : "Deposit Received");
  connect(pmReady, prereqJoin, "PM Ready");

  const scopeProject = add("scope_project");
  connect(prereqJoin, scopeProject, "All prereqs complete");
  ...
  const productionSlotDecision = add("production_slot_decision");
  connect(pmFinalizeSchedule, productionSlotDecision);
  const usePreassignedSlot = add("use_preassigned_production_slot");
  connect(productionSlotDecision, usePreassignedSlot, "Yes");
  const assignProductionSlot = add("assign_production_slot");
  connect(productionSlotDecision, assignProductionSlot, "No");

  const inProduction = add("enter_production");
  connect(usePreassignedSlot, inProduction);
  connect(assignProductionSlot, inProduction);
...
function drawChart(layout) {
  ...
  svg.setAttribute("width", String(Math.max(1, Math.round(layout.width * currentZoom))));
  svg.setAttribute("height", String(Math.max(1, Math.round(layout.height * currentZoom))));

  drawDefs(svg);
  const laneHeaderGroup = drawLanes(svg, layout);
  drawStages(svg, layout);
  drawEdges(svg, layout);
  drawNodes(svg, layout);
  if (laneHeaderGroup) {
    svg.append(laneHeaderGroup);
  }
  updateZoomUi();
  updateStickyLaneHeaders();
}
...
function drawLanes(svg, layout) {
  const laneHeaderGroup = createSvg("g", {
    id: LANE_HEADER_GROUP_ID,
    class: "lane-header-group"
  });
  ...
  layout.data.lanes.forEach((lane, index) => {
    ...
    laneHeaderGroup.append(createSvg("rect", {
      x: 0,
      y,
    ...
    laneHeaderGroup.append(title);
  });

  return laneHeaderGroup;
}
...
function drawNodes(svg, layout) {
  layout.nodes.forEach((node) => {
    ...
    group.setAttribute("aria-label", `Open RACI details for ${node.label}`);
    ...
    const openDetails = () => openRaciModal(node);
